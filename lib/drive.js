    ...
    for (const item of keys) {
        try {
            if (authCache.has(item.source)) {
                clients.push({
                    instance: google.drive({ version: 'v3', auth: authCache.get(item.source) }),
                    source: item.source
                });
                continue;
            }
            ...
            let credentials;
            try {
                // Try Base64 first (Vercel standard)
                credentials = JSON.parse(Buffer.from(item.key, 'base64').toString('utf8'));
                console.log(`[Drive Lib] Decoded Base64 credentials for ${item.source}`);
            } catch (e) {
                // If not base64, try raw JSON
                try {
                    credentials = JSON.parse(item.key);
                    console.log(`[Drive Lib] Parsed raw JSON credentials for ${item.source}`);
                } catch (errJson) {
                    console.error(`❌ [Drive Lib] Failed to parse credentials for ${item.source}. Check if it is a valid JSON or Base64 string.`);
                    continue;
                }
            }
            ...
            console.error(`❌ [Drive Lib] Failed to initialize client for ${item.source}: ${err.message}`);
        }
    }
