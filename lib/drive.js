...
export async function getDriveClient() {
    ...
    try {
        ...
        // Option 1: For Vercel/Env var - use GOOGLE_SERVICE_ACCOUNT_JSON (base64 encoded)
        if (process.env.GOOGLE_SERVICE_ACCOUNT_JSON) {
            try {
                ...
                console.log("✅ [Drive Lib] Using GOOGLE_SERVICE_ACCOUNT_JSON (base64)");
            } catch (parseError) {
                try {
                    const credentials = JSON.parse(process.env.GOOGLE_SERVICE_ACCOUNT_JSON);
                    auth = new google.auth.GoogleAuth({ credentials, scopes: SCOPES });
                    console.log("✅ [Drive Lib] Using GOOGLE_SERVICE_ACCOUNT_JSON (raw JSON)");
                } catch (e) {
                    console.error("❌ [Drive Lib] Failed to parse GOOGLE_SERVICE_ACCOUNT_JSON");
                }
            }
        }
            ...
            console.log("✅ [Drive Lib] Using GOOGLE_APPLICATION_CREDENTIALS path:", process.env.GOOGLE_APPLICATION_CREDENTIALS);
