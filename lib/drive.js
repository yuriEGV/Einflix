...
/**
 * Returns an array of all available Drive clients for retry logic.
 */
export async function getAllDriveClients() {
    ...
    for (const item of keys) {
        try {
            ...
            console.error(`❌ [Drive Lib] Failed to initialize client for ${item.source}`);
        }
    }

    // Also include standard/local if no Slot keys found?
    // For simplicity, we mostly rely on Slots for rotation.
